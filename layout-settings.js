...
(() => {
  ...
  const style = `
    ...
    .dhost-layout-settings-root .dhost-layout-card,
    .dhost-layout-settings-root .dhost-order-card{margin:0 0 7px}
  ...
  function botCount() {
    const stateCount = Array.isArray(window.STATE?.bots) ? window.STATE.bots.length : 0;
    if (stateCount) return stateCount;
    return document.querySelectorAll('.home-screen .bot-list > .bot-card').length;
  }

  function removeCompetingRows() {
    document.querySelectorAll(
      '.layout-setting-row, .layout-order-row, .dhost-layout-section, .dhost-layout-card, .dhost-order-card, #dhost-layout-row, #dhost-order-row'
    ).forEach(el => {
  ...
  function renderSettingsControls() {
    ...
    const cards = [...document.querySelectorAll('.screen > .list-card')].filter(c => !c.closest('#' + ROOT_ID));
    ...
    card.innerHTML = `<div class="list-row">
      ...
      <div class="dhost-layout-columns">
        ${[1,2,3,4].map(n => `<button type="button" class="dhost-layout-column ${columns === n ? 'active' : ''}" data-columns="${n}">${n}</button>`).join('')}
      </div>
    </div>`;

    const orderCard = document.createElement('div');
    orderCard.className = 'list-card dhost-order-card';
    orderCard.innerHTML = `<button type="button" class="list-row dhost-order-row">
      <div class="list-row-label">${LANG === 'en' ? 'Change order' : 'Изменить порядок'}</div>
      <span class="dhost-order-arrow"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg></span>
    </button>`;

    root.append(section, card, orderCard);
    target.after(root);
